...
exports.forgetPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(new ApiError("User Not found!. please try again", 404));
  }
...
`;
  ...
  try {
    ...
    await user.save();
    return next(new ApiError("Failed to send the email"));
  }
...
exports.verfiyResetCode = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(new ApiError("Code inValid or Expired", 500));
  }
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user.ResetPasswordCodeIsValid) {
    return next(new ApiError("reset code is not valid !!", 404));
  }
